...
config_root = '/Users/smin/PycharmProjects/1QCT/'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
...
token_tmp = config_root + 'KISREAL' + datetime.today().strftime("%Y%m%d")  # 토큰 로컬저장시 파일명 년월일
...
with open(config_root + 'kis_devlp_real.yaml', encoding='UTF-8') as f:
...
def _setTRENV(cfg):
    ...
    d = {
        ...
        'my_url': 'https://openapivts.koreainvestment.com:9443'  # 실전 도메인 (https://openapi.koreainvestment.com:9443)
...
def auth(svr='prod', product=_cfg['my_prod'], url=None):
    ...
    if saved_token is None:  # 기존 발급 토큰 확인이 안되면 발급처리944
...
#     else:
#         res = requests.get(url, headers=headers, params=params, verify=False)
#         # Instead of printing, store the result in variables
#         status_code = res.status_code
#         response_body = res.text
#         # Return these for further use
